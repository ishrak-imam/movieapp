...
export const login = createReducer(LOGIN_INITIAL_STATE, {
  ...
  [LOGIN_FAIL]: (state, payload) => mergeMapDeep(state, getMap({loading: false, fbLoading: false, error: getMap(payload)})),
...
export const register = createReducer(REGISTER_INITIAL_STATE, {
  ...
  [REGISTER_FAIL]: (state, payload) => mergeMapDeep(state, getMap({loading: false, error: getMap(payload)}))
});
